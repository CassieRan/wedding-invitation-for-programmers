<template>
    ...
    >
        ...
        <p
            ...
            ref="barrageFirst"
            :style="{
                transform: 'translate(' + initialOffset + 'px)',
                top: '10px'
            }"
        >
        ...
        >
            <span
                v-for="(item, index) in filterBarrage(barrages, 1)"
                :key="index"
                >{{ item }}</span
            >
        </p>
        ...
        >
            <span
                v-for="(item, index) in filterBarrage(barrages, 2)"
                :key="index"
                >{{ item }}</span
            >
        </p>
        ...
        >
            <span
                v-for="(item, index) in filterBarrage(barrages, 3)"
                :key="index"
                >{{ item }}</span
            >
        </p>
...
export default {
    ...
    methods: {
        ...
        getWidth(ref) {
            return (
                window.getComputedStyle(ref, null).width.replace('px', '') - 0
            )
        },
        async gitWishes() {
            const res = await axios.get('/api/blessing/all')
            if (res.data.code === 0) {
